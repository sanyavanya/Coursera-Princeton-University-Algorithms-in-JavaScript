...
const threeSumQuadratic = (a) => {
	quickSort(a, 0, a.length-1);
	console.time('timerQuadratic');
	let cnt = 0;
	for (i = 0; i < a.length - 2; i++) {
		...
		while (j != k) {
			...
			if (currentSum === 0) { cnt++; k-- }
			else if (currentSum > 0) k--;
			else if (currentSum < 0) j++; 
		}
	}
	console.timeEnd('timerQuadratic');
	return cnt;
}
...
const testClient = () => {
	fileToPromiseArray('./txt/8Kints.txt')
		.then(data => {
			//console.log(threeSum(data));
			//console.log(threeSumSorted(data));
			console.log(threeSumQuadratic(data));
		});
}
